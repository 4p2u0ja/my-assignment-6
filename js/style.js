...
displayVdo(vdo);
 ...
 const displayVdo = vdo =>{
    // console.log(vdo);
    const vdoDIV = document.getElementById("vdo-card");
    vdo.forEach( video => {
         console.log(video);
      const vdoCard = document . createElement(`div`);
      vdoCard.classList=`card w-80 bg-base-100 p-3 `;
      ...
      <div class="card-body">
        ...
        <figure><img src="${video.authors[0].profile_picture}" alt="" class=" w-10 rounded-full"/></figure>

        <p class=" mt-2">${video.authors[0].profile_name}</p>
 ...
 const allvdo=()=>{
    ...
    const all2 = document.getElementById("all-vdo");
    if(all2){
...
//     if(music== `video.category_id:1001`){
//          displayVdo();
//     }
// }
const comedy = () =>{
   
    const funFact =document.getElementById('fun');
    if(funFact)
    {
...
const blog = () =>{
    ...
    const other =document.getElementById('box');
    if(other)
    {
